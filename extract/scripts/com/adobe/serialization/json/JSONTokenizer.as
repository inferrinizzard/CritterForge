package com.adobe.serialization.json
{
   public class JSONTokenizer
   {
      ...
      private var useOptimizations:Boolean;
      
      public function JSONTokenizer(param1:String, param2:Boolean = false)
      {
         super();
         this.jsonString = param1;
         this.loc = 0;
         this.useOptimizations = param2;
         this.nextChar();
      }
         ...
         switch(this.ch)
         {
            ...
            case "t":
               if(this.useOptimizations)
               {
                  ...
                  _loc1_.value = true;
                  this.loc += 4;
                  this.ch = this.jsonString.charAt(this.loc - 1);
                  break;
               }
      ...
      private function readString() : JSONToken
      {
         ...
         if(this.useOptimizations)
         {
            _loc3_ = this.jsonString.indexOf("\"",int(this.loc - 1));
            if(_loc3_ == -1)
            {
               this.parseError("Unterminated string literal");
            }
            while(this.jsonString.charAt(int(_loc3_ - 1)) == "\\")
            {
               _loc3_ = this.jsonString.indexOf("\"",int(_loc3_ + 1));
            }
            if((_loc4_ = this.jsonString.slice(int(this.loc - 1),_loc3_)).indexOf("\\") < 0)
            {
               this.loc = int(_loc3_ + 1);
               this.nextChar();
               _loc1_.value = _loc4_;
               return _loc1_;
            }
            ...
            if(this.ch == "\\")
            {
               ...
               switch(this.ch)
               {
                  ...
                  case "u":
                     ...
                     _loc2_ += String.fromCharCode(parseInt(_loc5_,16));
                     break;
                  default:
                     _loc2_ += "\\" + this.ch;
               }
      ...
      private function readNumber() : JSONToken
      {
         ...
         if(this.useOptimizations)
         {
            _loc5_ = (_loc4_ = this.jsonString.slice(this.loc - 1,this.loc + 32)).indexOf(",");
            _loc6_ = _loc4_.indexOf("]");
            _loc7_ = _loc4_.indexOf("}");
            if(_loc5_ >= 0 && (_loc6_ == -1 || _loc5_ < _loc6_) && (_loc7_ == -1 || _loc5_ < _loc7_))
            {
               _loc8_ = _loc5_;
            }
            else if(_loc6_ >= 0 && (_loc5_ == -1 || _loc6_ < _loc5_) && (_loc7_ == -1 || _loc6_ < _loc7_))
            {
               _loc8_ = _loc6_;
            }
            else if(_loc7_ >= 0 && (_loc5_ == -1 || _loc7_ < _loc5_) && (_loc6_ == -1 || _loc7_ < _loc6_))
            {
               _loc8_ = _loc7_;
            }
            else
            ...
            _loc2_ = _loc4_.slice(0,_loc8_);
            this.loc += _loc2_.length;
            this.ch = this.jsonString.charAt(this.loc - 1);
         }
         else
         {
